            ...
            if (C.GetId() == clerk_id) {
                for (int j = 0; j < UsersList.size(); j++) {
                    ...
                    if (borrower_id == U.GetId()) {
                        C.CheckInItem(ret_date, book_id, U, this);
                    }
                }
            }
        }
    ...
    public void RenewBook(int book_id, int type) {
      ...
      void ClerkCheckOutItem( int book_id, int borrower_id, int clerk_id) {
        ...
        for (int i = 0; i < ClerkList.size(); i++) {
            ...
            if (C.GetId() == clerk_id) {
                //===============
                Users borrower = getUserByID(borrower_id);
                //get a new loan id
                int index=getNewLoanID();
                //get a new loan obj
                Loan LoanObj= new Loan(index);
                C.CheckOutItem(book_id , borrower, BooksList , LoanObj,  LoanList );
                //================
            }
        }
    }
